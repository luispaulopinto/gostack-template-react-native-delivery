...
const FoodDetails: React.FC = () => {
  ...
  const cartTotal = useMemo(() => {
    // Calculate cartTotal
    if (foodQuantity === 0) return 0;

    const totalExtra = extras.reduce((acc, cur) => {
      return acc + cur.value * cur.quantity;
    }, 0);

    const total = food.price * foodQuantity + totalExtra * foodQuantity;
  ...
  async function handleFinishOrder(): Promise<void> {
    ...
    const foodFormatted = { ...food, extras };

    delete foodFormatted.id;

    await api.post('orders', foodFormatted).catch(err => console.error(err));
  }
